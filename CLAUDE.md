...
| **NOTHING REACHES master EXCEPT VIA PR** | Never commit or push to `master` — branch, PR, let CI run. No carve-out for doc or typo fixes. This covers *any* refspec landing on trunk, including `git push origin <branch>:master`, which is a CLI merge however it is spelled. Never push --force or rebase `master`. | **BLOCKED** server-side (branch protection, `enforce_admins=true`) and locally (`main_branch_guard`) |
